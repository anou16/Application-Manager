...
/**
 * Container for the ApplicationManager that has the menu options for new
 * application files, loading existing files, saving files and quitting.
 * Depending on user actions, other JPanels are loaded for the different ways
...
public class AppManagerGUI extends JFrame implements ActionListener {
	 ...
	 * applications or for quitting the application.
	 */
	...
	private class AppListPanel extends JPanel implements ActionListener {
		...
		private class AppTableModel extends AbstractTableModel {
			...
			private String[] columnNames = { "Application ID", "Application State", "Application Type",
					"Application Summary" };
			...
			/**
			 * Updates the given model with Application information from the
			 * ApplicationManager.
			 */
			...
			/**
			 * Updates the given model with Application information for the given
			 * application type from the applicationManager.
			 * 
	...
	private class ReviewPanel extends JPanel implements ActionListener {
		...
		public ReviewPanel() {
			...
			JScrollPane scrollNote = new JScrollPane(txtNote, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == btnAccept) {
				if (reviewerId == null || !chkProcessed.isSelected()) {
					// If reviewer id is invalid and/or process paperwork checkbox is unchecked,
				...
				// Try a command. If problem, go back to application list.
				if (reviewerId == null || chkProcessed.isSelected()) {
					// If reviewer id is invalid and/or process paperwork checkbox is checked, show
					// an error message
					JOptionPane.showMessageDialog(AppManagerGUI.this, "Invalid information.");
					reset = false;
				} else {
	...
	/**
	 * Inner class that creates the look and behavior for the JPanel that interacts
	 * with a reference check application.
	 * 
	 * @author Dr. Sarah Heckman (sarah_heckman@ncsu.edu)
	 * @author Dr. Chandrika Satyavolu (jsatyav@ncsu.edu)
	 */
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == btnReopen) {
				...
				if (type.contentEquals(Application.A_OLD) && (!chkProcessed.isSelected() || reviewerId == null)) {
					...
					// If application type is new and process paperwork checkbox is checked, show an
					// error message
					JOptionPane.showMessageDialog(AppManagerGUI.this, "Invalid information.");
					reset = false;
				} else {
	...
	private class AppInfoPanel extends JPanel {
		...
		public AppInfoPanel() {
			...
			JScrollPane summaryScrollPane = new JScrollPane(txtSummary, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
					JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
			JScrollPane notesScrollPane = new JScrollPane(txtNotes, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
					JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
	...
	private class AddAppPanel extends JPanel implements ActionListener {
		...
		/**
		 * Creates the JPanel for adding new applications to the manager.
		 */
		public AddAppPanel() {
			super(new GridBagLayout());
